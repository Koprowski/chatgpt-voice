...
- **Config and runtime data** (config.json, Chrome profile, PID files, logs) live in platform directories (`%LOCALAPPDATA%\chatgpt-voice` on Windows, `~/.config/chatgpt-voice` on Linux, etc.) and are not part of this repository.
